...
export default async function handler(req, res) {
  const { hasQuadraticVoting, name, ownerAddress, tokenCap, tokenName, tokenSymbol, userAddress } = req.body;
  // deployer address
  console.log(`--- \n\nADDR1: ${userAddress}`);
  ...
  const token = await Token.deploy(userAddress, tokenName, tokenSymbol, tokenCap, hasQuadraticVoting);
  ...
  const governorAddr = ethers.utils.getContractAddress({ from: userAddress, nonce: nonce + 1 });
  ...
  const govContract = await GovernorAlpha.deploy(timelock.address, token.address, userAddress, name);
  ...
  dao.tokenAbi = JSON.stringify(abiTk);
  dao.contractAbi = JSON.stringify(abiGv);
  await dao.save();
